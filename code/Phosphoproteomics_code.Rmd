...
RequiredPackages <- c("BiocManager", "dplyr", "igraph", "tidyr") 
...
- There is also a link to the [STRINGdb website for less stringent threshold = 1 ](https://version-11-0b.string-db.org/cgi/link?to=8602E81F655D156B) with the Proteins connected to Stat5b in this dataset.
- There is also a link to the [STRINGdb website for more stringent threshold = 200 ](https://version-11-0b.string-db.org/cgi/link?to=57B67172C2751CB6) with the Proteins connected to Stat5b in this dataset.
...
print(pathways_less[1:5,c(3,4,10)])
#Table for more stringent parameters
print(pathways_more[1:5,c(3,4,10)])
#Identifying pathways also for Proteins with a ratio >=2 and p.value <= 0.05 and threshold = 200.
enrichment <- string_db_high$get_enrichment(more_phosphorylated, category = "KEGG")
print(enrichment[1:5,c(3,4,10)])
#Identifying pathways also for Proteins with a ratio >=2 and p.value <= 0.05 and threshold = 1.
enrichment <- string_db_low$get_enrichment(more_phosphorylated, category = "KEGG")
print(enrichment[1:5,c(3,4,10)])
