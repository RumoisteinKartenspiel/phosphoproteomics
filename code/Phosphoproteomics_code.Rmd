...
-   Here we use the identified thresholds from the volcano plots to clean the data. First we select rows with ratio >= 2 and p.values <= 0.05 from the phopho dataset. From the proteom data set values were selected with a all rows with a ratio <= 1, all rows with a ratio >= 1 >= 2 and p.value >= 0.05 (significant p-values are excluded and ratios > bigger than 2 too). After this initial cleanup a semi_join() was performed return all rows from cleanup phospho with a match in cleanup proteome, to select only phospo values that showed increased phosphorylation due to the LIF treatment.
