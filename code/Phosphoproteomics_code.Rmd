...
- Here just the data.frame is subset

```{r}
more_phosphorylated <- subset.data.frame(data_phospho, subset = data_phospho$Abudance.Ratio >= 2)
more_phosphorylated <- subset.data.frame(more_phosphorylated, subset = more_phosphorylated$Abudance.pvalue <= 0.05)
remove(data_phospho)
```
...
Proteins_connected_to_Stat5b <- string_db$add_proteins_description(gh) #Add´s protein information to STRINGid´s 
old_end <- Sys.time()
old_end-old_start
...
Proteins_connected_to_Stat5b_quick_way <- string_db$add_proteins_description(Proteins_connected_to_Stat5b_quick_way) #Add´s protein information to STRINGid´s
new_end <- Sys.time()
new_end-new_start
